...
RELEASE = ROOT / "versioned" / "spo-online-16.0.27711.12757"
...
class ExactBuild2771112757ReleaseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.authority = validator.load_json(
            RELEASE / "authority" / "spo-online-16.0.27711.12757.authority.json"
        )
        ...
        cls.profile = validator.load_json(
            RELEASE / "profile" / "spo-online-16.0.27711.12757.profile.json"
        )
        cls.acquisition_profile = validator.load_json(
            RELEASE
            / "profile"
            / "spo-online-16.0.27711.12757-acquisition-consumer.profile.json"
        )
        ...
        cls.provenance = validator.load_json(RELEASE / "source-provenance.json")
    ...
    def test_exact_authority_registry_and_profiles_are_closed(self) -> None:
        ...
        self.assertEqual(
            "0ff1276087a43640144d8072fa347a886473855f",
            self.authority["authoritySourceRef"],
        )
        self.assertEqual(
            "release/16.0.27711.12757", self.authority["authoritySourceTag"]
        )
        self.assertEqual("2026-09-12T01:49:39Z", self.authority["authorityCommitTime"])
        self.assertEqual(1156, self.registry["entryCount"])
        self.assertEqual(
            "6c992a8ae11e186bf5fdaba77c082a7c9a893f99",
            self.provenance["authorityTree"],
        )

    def test_build_binding_is_exact_and_never_becomes_a_range(self) -> None:
        self.assertEqual("16.0.27711.12757", self.registry["platformBuildMin"])
        self.assertEqual("16.0.27711.12757", self.registry["platformBuildMax"])
        self.assertEqual("16.0.27711.12757", self.profile["platformBuild"])
        schema_hash = validator.artifact_sha256(self.registry_schema_path)
        for build, reason in [
            ("unknown", "UNKNOWN_PLATFORM_BUILD"),
            ("16.0.27709.12001", "INCOMPATIBLE_PLATFORM_BUILD"),
            ("16.0.27711.12756", "INCOMPATIBLE_PLATFORM_BUILD"),
            ("16.0.27711.12758", "INCOMPATIBLE_PLATFORM_BUILD"),
        ]:
    ...
    def test_setup_virtual_and_acquisition_volumes_remain_separate(self) -> None:
        ...
        self.assertEqual(
            11,
            sum(
                entry["downstreamDisposition"] == "ReferenceUnavailable"
                for entry in self.registry["entries"]
            ),
        )
        self.assertEqual(
    ...
    def test_new_release_is_not_derived_from_the_previous_registry(self) -> None:
        prior = validator.load_json(
            ROOT
            / "versioned"
            / "spo-online-16.0.27709.12001"
            / "authority"
            / "spo-online-16.0.27709.12001.authority.json"
        )
        self.assertNotEqual(
