...
namespace PnP.Framework.Migration.Topology.Ingredients
{
    public static class PathDerivedSourceTopologyEvidenceFactory
    {
        ...
        {
            var authorization = new TopologyHttpFailureEvidence
            {
                Operation = operation,
            ...
            authorization.EvidenceSha256 = SharedTopologyDigest.ComputeHttpFailure(authorization);
        ...
        public static void Validate(PathDerivedSourceTopologyEvidence evidence)
        {
            ...
            {
                ValidateAuthorization(evidence.AuthorizationEvidence);
            }
            else if (evidence.AuthorizationEvidence != null)
            {
                throw new InvalidDataException("Only AuthorizationBlocked source fidelity may carry literal authorization evidence.");
            }
        ...
        private static void ValidateAuthorization(TopologyHttpFailureEvidence evidence)
        {
            if (evidence == null
                || string.IsNullOrWhiteSpace(evidence.Operation)
                || !IsLiteralAuthorizationStatus(evidence.HttpStatusCode)
                || evidence.ObservedAtUtc == default(DateTimeOffset))
            {
                throw new InvalidDataException("AuthorizationBlocked source fidelity requires retained literal HTTP 401/403 evidence.");
            }
            try
            {
            ...
            {
                throw new InvalidDataException("Authorization evidence contains an invalid request URI.", exception);
            }
            if (!IsSha256(evidence.EvidenceSha256)
                || !string.Equals(evidence.EvidenceSha256, SharedTopologyDigest.ComputeHttpFailure(evidence), StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidDataException("Authorization evidence digest does not match its literal wire facts.");
            }
        }
