...
namespace PnP.Framework.Migration.Topology.Ingredients
{
    ...
    {
        private static readonly char[] ReservedSegmentCharacters = ":/?#[]@!$&'()*+,;=".ToCharArray();
        ...
        public static string NormalizeServerRelativePath(string value, string parameterName)
        {
            ...
            if (trimmed.IndexOf('?') >= 0
                ...
                || trimmed.IndexOf("%25", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                throw new ArgumentException("A server-relative path cannot contain a query, fragment, backslash, encoded delimiter, or double-encoding marker.", parameterName);
            }
            string normalized;
            try
            {
                normalized = Uri.UnescapeDataString(trimmed).Normalize(NormalizationForm.FormC);
            }
            catch (UriFormatException exception)
            {
                throw new ArgumentException("The server-relative path contains invalid percent encoding.", parameterName, exception);
            }
            if (normalized.Length > 2048
                || normalized.IndexOf('%') >= 0
                || normalized.Any(char.IsControl))
            {
                throw new ArgumentException("The server-relative path is too long or contains an ambiguous/control character.", parameterName);
            }
            ...
            try
            {
                ...
                {
                    ValidateSegment(segment, parameterName);
                }
            }
            catch (ArgumentException exception)
            {
                throw new ArgumentException("The decoded path contains an empty, traversing, reserved, or ambiguous segment.", parameterName, exception);
            }
            return normalized;
        }
        ...
        public static string Combine(string parent, string segment)
        {
            ...
            return (normalizedParent.Length == 0 ? string.Empty : normalizedParent) + "/" + segment.Normalize(NormalizationForm.FormC);
        }
        ...
        private static void ValidateSegment(string segment, string parameterName)
        {
            ...
            if (string.IsNullOrWhiteSpace(normalized)
                ...
                || normalized.IndexOf('#') >= 0
                || normalized.IndexOf('%') >= 0
                || normalized.IndexOfAny(ReservedSegmentCharacters) >= 0
                || normalized.Any(char.IsControl)
