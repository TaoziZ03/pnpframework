...
namespace PnP.Framework.Migration.Pages.Assessment.Maturity
{
    internal static class IngredientMaturityEvidenceValidator
    {
        ...
        {
            // Preserve the non-runtime path, but required runtime needs the
            // consumer's independent producer binding, never the receipt's own ref.
            return ValidateM4(null, evidence);
        }

        public static IReadOnlyList<IngredientMaturityGateReceipt> ValidateM4(
            IngredientMaturityEvaluationContext context,
            IngredientOperationalEvidence evidence)
        {
            return new[]
            {
                ...
                {
                    Require(evidence?.ImportReceipt != null && evidence.ActionSignature != null
                ...
                Validate(IngredientMaturityGateCatalog.RuntimeCleanupRetry, evidence?.RuntimeCleanupRetryEvidenceReferences, () =>
                {
                    ...
                    {
                        RequireRuntimeEvidence(context, evidence);
                    }
                    else
                    {
                        Require(evidence.ImportReceipt != null
                            && (evidence.ImportReceipt.RuntimeVerificationStatus == RuntimeVerificationStatus.NotRequired
        ...
        private static void RequireRuntimeEvidence(
            ...
            IngredientOperationalEvidence evidence)
        {
            Require(evidence.RuntimeReceipt != null, "Required runtime verification is absent.");
            IngredientMaturityEvaluator.ValidateContext(context);
            Require(evidence.Plan != null && evidence.ActionSignature != null
                && IngredientMaturityEvaluator.IsSha256(evidence.AdmittedPlanDigest)
                && string.Equals(PublishingPageDigest.ComputePlanDigest(evidence.Plan),
                    evidence.AdmittedPlanDigest, StringComparison.OrdinalIgnoreCase),
                "Required runtime evidence must bind the independently recomputed page plan.");

            // The page-plan digest and the admitted execution-envelope digest are
            // different contracts. Recompute both before joining native receipts.
            var admissionDigest = AdmittedReproExecutionPlanValidator.ValidateAndComputeDigest(
                evidence.AdmittedExecutionPlan, evidence.AdmittedPlanDigest, evidence.ActionSignature.TargetIdentity);
            Require(string.Equals(admissionDigest, evidence.AdmittedExecutionPlanDigestSha256, StringComparison.OrdinalIgnoreCase),
                "The admitted execution-plan digest is missing, foreign, or stale.");
            PublishingPageImportReceiptValidator.ValidateAdmittedExecution(
                evidence.ImportReceipt, evidence.AdmittedExecutionPlan, evidence.AdmittedExecutionPlanDigestSha256);
            var importDigest = MigrationDigest.ComputeSha256(MigrationContractSerializer.SerializeCanonical(evidence.ImportReceipt));

            var manifest = evidence.Plan.RuntimeVerification;
            var receipt = evidence.RuntimeReceipt;
            var admitted = evidence.AdmittedExecutionPlan;
            Require(manifest != null
                ...
                && string.Equals(receipt.SchemaVersion, PublishingPageCompareContract.RuntimeReceiptSchemaVersion, StringComparison.Ordinal),
                "The required runtime manifest or receipt schema is unsupported.");
            Require(string.Equals(receipt.PlanDigest, evidence.AdmittedPlanDigest, StringComparison.OrdinalIgnoreCase)
                && string.Equals(receipt.AdmittedPlanDigestSha256, admissionDigest, StringComparison.OrdinalIgnoreCase)
                && receipt.OperationId == admitted.Operations.RuntimeOperationId
                && AdmittedReproExecutionPlanValidator.SameOperations(receipt.Operations, admitted.Operations)
                && AdmittedReproExecutionPlanValidator.SameSourceVersion(receipt.SourceVersion, admitted.SourceVersion)
                && string.Equals(receipt.ImportReceiptDigestSha256, importDigest, StringComparison.OrdinalIgnoreCase)
                && string.Equals(receipt.TargetIdentity, evidence.ActionSignature.TargetIdentity, StringComparison.Ordinal),
                "The runtime receipt is not bound to the admitted plan, native import, source version, operations, and target.");
            RequireRuntimeTimeline(evidence);

            // Reuse D's implementation, manifest, browser, HTTP/cache, safe locator,
            // and exact HTML/DOM/optional screenshot byte validation without changing
            // its public schema or recreating its artifact-resolution contract.
            try
            {
                RuntimeVerificationReceiptValidator.ValidateEvidence(
                    receipt, manifest, context.Producer.ImplementationCommit, evidence.RuntimeArtifactStore);
            }
            catch (Exception exception) when (exception is IOException || exception is UnauthorizedAccessException)
            {
                throw new InvalidDataException("Required runtime artifacts could not be read.", exception);
            }
            foreach (var result in receipt.Results ?? new List<RuntimeVerificationResult>())
            {
                RequireUtc(result.Http.CapturedAtUtc, "runtime HTTP capture");
            }

            Require(manifest.Requirements != null
                && manifest.Requirements.All(value => value != null && !string.IsNullOrWhiteSpace(value.Id))
                && manifest.Requirements.Select(value => value.Id).Distinct(StringComparer.Ordinal).Count() == manifest.Requirements.Count,
                "Runtime manifest requirement IDs must be nonempty and unique.");
            ...
            var required = manifest.Requirements.Where(value => value.Required).ToArray();
            Require(required.All(requirement => results.Count(value =>
                    string.Equals(value.RequirementId, requirement.Id, StringComparison.Ordinal)) == 1),
                "The runtime receipt must cover every required requirement exactly once.");
            var expectedStatus = required.Length == 0
                ? RuntimeVerificationStatus.NotRequired
                : required.All(requirement => results.Single(value =>
            ...
            Require(receipt.Status == expectedStatus,
                "The runtime receipt status does not agree with required results.");
            Require(expectedStatus == RuntimeVerificationStatus.Passed,
                "Required runtime verification is pending, failed, or has no required results.");
        }

        private static void RequireRuntimeTimeline(IngredientOperationalEvidence evidence)
        {
            var receipt = evidence.RuntimeReceipt;
            var import = evidence.ImportReceipt;
            var browser = receipt.BrowserContext;
            RequireUtc(evidence.ExecutionStartedAtUtc, "execution start");
            RequireUtc(import.StartedAtUtc, "native import start");
            RequireUtc(import.CompletedAtUtc, "native import completion");
            RequireUtc(receipt.CompletedAtUtc, "runtime completion");
            Require(evidence.JournalState != null && evidence.VerificationReceipt != null,
                "Runtime evidence requires its execution journal and verification receipt.");
            RequireUtc(evidence.JournalState.RecordedAtUtc, "execution journal recording");
            RequireUtc(evidence.VerificationReceipt.VerifiedAtUtc, "mutation verification");
            Require(browser != null, "A runtime browser context is required.");
            RequireUtc(browser.CreatedAtUtc, "browser context creation");
            RequireUtc(browser.FirstNavigationAtUtc, "browser first navigation");
            Require(import.StartedAtUtc >= evidence.ExecutionStartedAtUtc
                && receipt.CompletedAtUtc >= evidence.ExecutionStartedAtUtc
                && browser.CreatedAtUtc >= evidence.ExecutionStartedAtUtc
                && browser.FirstNavigationAtUtc >= import.CompletedAtUtc,
                "Runtime evidence must use a current-execution browser and post-import navigation.");
            foreach (var step in import.Steps)
            {
                RequireUtc(step.CompletedAtUtc, "native import step");
            }
            foreach (var source in new[] { evidence.AdmittedExecutionPlan.SourceVersion, import.SourceVersion, receipt.SourceVersion })
            {
                RequireUtc(source.ObservedAtUtc, "source-version observation");
                Require(source.ObservedAtUtc <= evidence.ExecutionStartedAtUtc,
                    "The admitted source-version observation must precede execution.");
                if (source.LastModifiedUtc.HasValue)
                {
                    RequireUtc(source.LastModifiedUtc.Value, "source last modification");
                    Require(source.LastModifiedUtc <= source.ObservedAtUtc,
                        "The source last-modification time cannot follow its observation.");
                }
            }
        }
        ...
        {
            Require(value != default && value.Offset == TimeSpan.Zero,
                "The " + name + " requires a non-default UTC timestamp.");
        }
