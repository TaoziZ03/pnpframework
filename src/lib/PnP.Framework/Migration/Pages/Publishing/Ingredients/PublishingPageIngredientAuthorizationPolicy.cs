...
namespace PnP.Framework.Migration.Pages.Publishing.Ingredients
{
    ...
    /// retained literal wire HTTP 401/403 evidence. Domain planner Block decisions
    /// are normalized to Defer before this policy is applied.
    /// </summary>
    internal static class PublishingPageIngredientAuthorizationPolicy
    {
            ...
            IDictionary<string, PageIngredientAction> actions)
        {
            foreach (var pair in GetEvidence(snapshot))
            {
                ...
                action.TerminalStatus = IngredientTerminalStatus.AuthorizationBlocked;
                action.AuthorizationStatusCode = evidence.HttpStatusCode;
