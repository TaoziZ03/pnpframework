...
{
    public sealed class WelcomePageDiscovery
    {
  ...
  <Query>
    <Where>
      <BeginsWith>
        ...
        <Value Type='ContentTypeId'>{BuiltInContentTypeId.WelcomePage}</Value>
      </BeginsWith>
    </Where>
  ...
  <RowLimit Paged='TRUE'>500</RowLimit>
                ...
                foreach (var item in items)
                {
                    ...
                    if (WelcomePageV1CohortPolicy.IsIncludedContentType(contentTypeId) && !string.IsNullOrWhiteSpace(fileRef))
                    {
                        result.Add(fileRef);
                    }
                }
        ...
        public static bool IsWelcomePageContentType(string contentTypeId)
        {
            return WelcomePageV1CohortPolicy.IsIncludedContentType(contentTypeId);
        }
    }
}
