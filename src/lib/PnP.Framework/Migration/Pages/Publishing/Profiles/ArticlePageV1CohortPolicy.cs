...
{
    public static class ArticlePageV1CohortPolicy
    {
        public const string CohortId = "article-page-v1";
        ...
        public static ValidationCohortAssessment Assess(string contentTypeId)
        {
            if (string.IsNullOrWhiteSpace(contentTypeId))
            {
                return Result(
                    ...
                    "The source ContentTypeId is unavailable, so Article-v1 cohort membership cannot be established.");
            }
            ...
            if (contentTypeId.StartsWith(BuiltInContentTypeId.EnterpriseWikiPage, StringComparison.OrdinalIgnoreCase))
            {
                return Result(
                    ...
                    "Enterprise Wiki Content Type lineage is handled by EW-v1 and excluded from the Article-v1 validation cohort.");
            }
            ...
            if (contentTypeId.StartsWith(BuiltInContentTypeId.WelcomePage, StringComparison.OrdinalIgnoreCase))
            {
                return Result(
                    ValidationCohortDisposition.Excluded,
                    "Welcome Page Content Type lineage is handled by Welcome-v1 and excluded from the Article-v1 validation cohort.");
            }
            ...
            if (contentTypeId.StartsWith(BuiltInContentTypeId.ProjectPage, StringComparison.OrdinalIgnoreCase))
            {
                return Result(
                    ValidationCohortDisposition.Excluded,
                    "Project Page Content Type lineage is intentionally outside the Article-v1 validation cohort; migration capability is assessed independently.");
            }
            ...
            if (contentTypeId.StartsWith(BuiltInContentTypeId.ArticlePage, StringComparison.OrdinalIgnoreCase))
            {
                return Result(
                    ...
                    "Article Page Content Type lineage is included by the Article-v1 validation policy.");
            }

            return Result(
                ValidationCohortDisposition.Excluded,
                "The source Content Type is outside the Article-v1 validation cohort.");
        }
