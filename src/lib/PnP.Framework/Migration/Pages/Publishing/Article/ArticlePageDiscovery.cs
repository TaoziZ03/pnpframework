...
{
    public sealed class ArticlePageDiscovery
    {
  ...
  <Query>
    <Where>
      <BeginsWith>
        ...
        <Value Type='ContentTypeId'>{BuiltInContentTypeId.ArticlePage}</Value>
      </BeginsWith>
    </Where>
  ...
  <RowLimit Paged='TRUE'>500</RowLimit>
                ...
                foreach (var item in items)
                {
                    ...
                    if (ArticlePageV1CohortPolicy.IsIncludedContentType(contentTypeId) && !string.IsNullOrWhiteSpace(fileRef))
                    {
                        result.Add(fileRef);
                    }
                }
        ...
        public static bool IsArticlePageContentType(string contentTypeId)
        {
            return ArticlePageV1CohortPolicy.IsIncludedContentType(contentTypeId);
        }
    }
}
