...
namespace PnP.Framework.Migration.Taxonomy.Assets.Execution
{
    internal static class TaxonomyAssetMaterializationCoordinator
    {
            ...
            MigrationExecutionRecorder recorder)
        {
            ...
            foreach (var actionReceipt in receipt.Actions)
            {
                TaxonomyAssetReceiptIdentity.Populate(
                    actionReceipt,
                    approved[actionReceipt.ActionId],
                    reviewedPlan.PlanDigest,
            ...
            RecordDurableCheckpoint(reviewedPlan, approval, receipt, catalog, approved, recorder);
                         ...
                         .ThenBy(value => value.Source.TermStoreId))
            {
                ...
                if (probe.Disposition == TaxonomyAssetTargetDisposition.ReuseOwned)
                {
                    recorder.RecordAlreadySatisfied(
                        MutationIdentity(recorder, approved[actionId]),
                        actionId,
                        "Fresh preflight approved reusable taxonomy TermGroup '" + plan.TargetGroupName + "'.");
                }
                else
                {
                         ...
                         .ThenBy(value => value.Source.TermSetId))
            {
                    ...
                    || probe.Disposition == TaxonomyAssetTargetDisposition.ReviewExternalReuse)
                {
                    recorder.RecordAlreadySatisfied(
                        MutationIdentity(recorder, approved[actionId]),
                        actionId,
                        "Fresh preflight approved reusable TermSet '" + plan.SourceTermSetName + "'.");
                }
                else
                {
                    changed = recorder.Execute(
            ...
            foreach (var plan in OrderTerms(reviewedPlan.Terms))
            {
                    ...
                    || probe.Disposition == TaxonomyAssetTargetDisposition.ReviewExternalReuse)
                {
                    recorder.RecordAlreadySatisfied(
                        MutationIdentity(recorder, approved[actionId]),
                        actionId,
                        "Fresh preflight approved reusable Term '" + plan.Name + "'.");
                }
                else
                {
                    changed = recorder.Execute(
        ...
        private static void RecordDurableCheckpoint(
            TaxonomyAssetReviewPlan reviewedPlan,
            TaxonomyAssetApprovalManifest approval,
            TaxonomyAssetMaterializationReceipt receipt,
            ...
            MigrationExecutionRecorder recorder)
        {
            foreach (var actionReceipt in receipt.Actions.OrderBy(value => value.ActionId, StringComparer.Ordinal))
            {
                var action = approved[actionReceipt.ActionId];
                var identity = MutationIdentity(recorder, action);
                recorder.RecordVerification(new MigrationMutationVerificationReceipt
                {
                    ...
                    CurrentStateDigest = identity.SemanticDigest,
                    Ownership = actionReceipt.Ownership.ToString(),
                    TargetIdentity = actionReceipt.TargetIdentity,
                    SourceSnapshotDigest = reviewedPlan.SourceSnapshotDigest,
                    ApprovalDigest = approval.ApprovalDigest,
                    IngredientId = identity.IngredientId,
                    SelectedDisposition = identity.SelectedDisposition,
                    TargetBoundaryDigest = recorder.Boundary.TargetBoundaryDigest,
            ...
            var receiptJson = MigrationContractSerializer.SerializeCanonical(receipt);
            recorder.RecordArtifact(new MigrationExecutionArtifact
            {
                OperationId = receipt.OperationId,
                PlanDigest = reviewedPlan.PlanDigest,
                WrittenAtUtc = receipt.CompletedAtUtc,
                ArtifactKind = "TaxonomyAssetMaterializationReceipt",
                ArtifactSchemaVersion = receipt.SchemaVersion,
                ArtifactDigest = receipt.ReceiptDigest,
                PayloadJson = receiptJson,
                PayloadSha256 = MigrationDigest.ComputeSha256(receiptJson)
            });
            var catalogJson = MigrationContractSerializer.SerializeCanonical(catalog);
            recorder.RecordArtifact(new MigrationExecutionArtifact
            {
                OperationId = receipt.OperationId,
                PlanDigest = reviewedPlan.PlanDigest,
                WrittenAtUtc = catalog.GeneratedAtUtc,
                ArtifactKind = "TaxonomyAssetMappingCatalog",
                ArtifactSchemaVersion = catalog.SchemaVersion,
                ArtifactDigest = catalog.CatalogDigest,
                PayloadJson = catalogJson,
                PayloadSha256 = MigrationDigest.ComputeSha256(catalogJson)
            });
        }
        ...
        private static MigrationMutationIdentity MutationIdentity(
            MigrationExecutionRecorder recorder,
            TaxonomyAssetActionApproval action)
        {
            var semanticDigest = TaxonomyAssetReceiptIdentity.SemanticMappingDigest(action);
            return MigrationMutationIdentity.Create(
                recorder.Boundary,
                action.ActionId,
                action.ActionId,
