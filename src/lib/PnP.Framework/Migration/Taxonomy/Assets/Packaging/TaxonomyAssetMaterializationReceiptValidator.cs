...
namespace PnP.Framework.Migration.Taxonomy.Assets.Packaging
{
    public static class TaxonomyAssetMaterializationReceiptValidator
    {
            ...
            ICollection<string> errors)
        {
            ...
            var expectedDisposition = approved.ReviewedDisposition == TaxonomyAssetTargetDisposition.ReviewExternalReuse
            ...
            if (actual.Ownership != expectedOwnership
                || actual.ExecutionDisposition != expectedDisposition
                ...
                || !string.Equals(actual.ReviewPlanDigest, reviewPlanDigest, StringComparison.OrdinalIgnoreCase)
                || !string.Equals(actual.ApprovalDigest, approvalDigest, StringComparison.OrdinalIgnoreCase))
            {
                errors.Add("Taxonomy receipt action '" + actual.ActionId + "' lacks its stable source/target mapping, ownership, disposition, or sealed approval boundary.");
            }
        }
