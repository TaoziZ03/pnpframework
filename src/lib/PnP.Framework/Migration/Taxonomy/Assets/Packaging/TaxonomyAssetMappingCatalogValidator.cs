...
namespace PnP.Framework.Migration.Taxonomy.Assets.Packaging
{
    public static class TaxonomyAssetMappingCatalogValidator
    {
        public static void Validate(TaxonomyAssetMappingCatalog catalog, bool requireDigest = true)
        {
            ...
            if (catalog.AssetMappings == null || catalog.AssetMappings.Count == 0)
            {
                errors.Add("The taxonomy asset mapping catalog has no verified asset mappings.");
            }
            foreach (var mapping in catalog.AssetMappings ?? new List<TaxonomyAssetMappingCatalogEntry>())
            {
                if (mapping == null
                    ...
                    || (mapping.Ownership == TaxonomyAssetOwnership.External
                        && mapping.Disposition != TaxonomyAssetReceiptDisposition.ReuseExternal)
                    || (mapping.Ownership == TaxonomyAssetOwnership.MigrationOwned
                        && mapping.Disposition == TaxonomyAssetReceiptDisposition.ReuseExternal))
                {
                    errors.Add("A taxonomy asset mapping entry is null, duplicate, unverified, or has inconsistent ownership/disposition evidence.");
                }
            }
