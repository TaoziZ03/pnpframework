    ...
    public static class TaxonomyAssetTargetInspector
    {
            ...
            if (ids.Length == 1)
            {
                ...
                if (result.Disposition == TaxonomyAssetTargetDisposition.ReconcileOwnedPlanDrift)
                {
                    result.Issues.Add(Issue(
                        ...
                        "The provenance-matched TermSet differs in semantic mapping provenance, name, open state, or tagging availability; reconcile it to the reviewed plan."));
                }
                return result;
            }
            if (!preferredExists)
            ...
            bool targetSetRequiresExternalApproval)
        {
            ...
            if (ids.Length == 1)
            {
                ...
                else if (provenanceState == TaxonomyOwnedProvenanceState.Exact
                    && ExactTermShape(match, plan)
            ...
            IEnumerable<TaxonomyTermTargetProbe> termProbes = null)
        {
            ...
            {
                assertions.Add("Fresh readback proves the source identity URN and stable semantic mapping digest are stored in the reserved owned-provenance properties.");
            }
