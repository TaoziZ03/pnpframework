...
Path-derived topology gives that scheduler a reusable ownership key: the canonical target-slot key identifies the mutually exclusive write lease, while the global action key identifies the approved signature for that slot. Workers may share an equivalent completed receipt, but two different signatures for one slot must be rejected before scheduling. The executor still protects correctness if leases expire: it freshly probes before each action, accepts only exact owned convergence, and never overwrites a foreign result.
