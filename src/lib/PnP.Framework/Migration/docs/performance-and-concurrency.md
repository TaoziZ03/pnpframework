...
Path-derived topology gives that scheduler a reusable ownership key: the canonical authority + Site fence + path target-slot key identifies the mutually exclusive write lease, while the logical action key identifies the stable producer. Exact per-capture `MigrationActionSignature` values are execution grants retained by that producer, not lease identities. The execution-group digest groups target-specific logical producers; a separate normalized support-cohort digest groups equivalent capabilities and target profiles across URLs and GUIDs. Workers may share one logical producer across capture timestamps or leaves, but incompatible logical semantics for one slot block before scheduling. The executor still protects correctness if leases expire: it freshly probes before each action, accepts only exact convergence, and never overwrites a foreign result.
