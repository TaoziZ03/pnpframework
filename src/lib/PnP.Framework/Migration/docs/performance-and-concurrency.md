...
Path-derived topology gives that scheduler a reusable ownership key: the canonical authority + Site fence + path target-slot key identifies the mutually exclusive write lease, while the generic `MigrationActionSignature` identifies the approved producer for that slot. A concrete execution-group digest groups target-specific producers; a separate normalized support-cohort digest groups equivalent capabilities and target profiles across URLs and GUIDs. Workers may share an equivalent verified receipt, but two different signatures for one slot must be rejected before scheduling. The executor still protects correctness if leases expire: it freshly probes before each action, accepts only exact convergence, and never overwrites a foreign result.
