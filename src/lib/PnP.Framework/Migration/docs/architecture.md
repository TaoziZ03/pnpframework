...
When the complete source ancestor-Web closure is denied by literal HTTP `401/403`, path-derived topology preserves captured root and leaf Webs and represents every unknown intermediate path as its own authorization-limited fidelity ingredient. The target root and each child path are independent global actions signed by the generic durable action contract. Cross-page plans union by authority/Site/path slot plus action signature. Equivalent producers deduplicate; different signatures for the same slot block globally. Pages pin and validate the complete shared producer proof and cannot execute a duplicate page-local Web create. See [Path-derived shared topology](path-derived-shared-topology.md).
