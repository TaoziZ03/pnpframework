...
When the complete source ancestor-Web closure is denied by literal HTTP `401/403`, path-derived topology keeps that source-fidelity ingredient authorization-limited while compiling each retained relative segment into a target-only container action. Cross-page plans union by target slot plus stable action signature. Equivalent producers deduplicate; different signatures for the same slot block globally. Pages reference the producer DAG and cannot execute a duplicate page-local Web create. See [Path-derived shared topology](path-derived-shared-topology.md).
