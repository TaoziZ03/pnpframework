...
| Continue exact target-path preparation without inventing denied source ancestor metadata. | `PathDerivedSourceTopologyEvidence`, per-segment `TargetWebContainerIngredientPlan`, a signature-conflict-checking global action DAG, and per-action fresh-readback receipts keep source fidelity `401/403` separate from target-only provisioning. |
